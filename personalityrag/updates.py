...
from .version import TAG_NAME, VERSION
...
class UpdateService:
    ...
    def _fallback_release_feed(cls) -> list[dict[str, Any]]:
        ...
        for entry in document.findall("atom:entry", namespace):
            ...
            try:
                ...
                continue
            expected_name = f"PersonalityRAG-{tag}.zip"
    ...
    def _normalize_release(raw: dict[str, Any]) -> dict[str, Any] | None:
        ...
        if raw.get("draft"):
            return None
        expected_asset = f"PersonalityRAG-{tag}.zip"
    ...
    def _download(url: str, destination: Path, expected_digest: str = "") -> str:
        ...
        with urllib.request.urlopen(request, timeout=60) as response, destination.open("wb") as output:
            ...
            while chunk := response.read(DOWNLOAD_CHUNK_BYTES):
                total += len(chunk)
                if total > MAX_WINDOWS_RELEASE_BYTES:
    ...
    async def prepare_switch(
        ...
        *,
        ...
        async with self._lock:
            ...
            if release is None:
                raise UpdatePackageError("target release is unavailable or lacks the Windows asset")
            expected_url = (
                f"https://github.com/Creeper3222/PersonalityRAG/releases/download/"
                f"{tag_name}/PersonalityRAG-{tag_name}.zip"
            )
            ...
            try:
                ...
                self._launch_helper(helper, transaction_file)
                return self._public_transaction(payload)
            except Exception:
                shutil.rmtree(transaction_root, ignore_errors=True)
                raise

    def _launch_helper(self, helper: Path, transaction_file: Path) -> None:
        command = [sys.executable, str(helper), "apply", str(transaction_file)]
        kwargs: dict[str, Any] = {
