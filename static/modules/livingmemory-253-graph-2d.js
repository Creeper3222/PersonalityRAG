(() => {
  ...
  /* LivingMemory 2.5.3 renderer, adapted for the host palette and prior hover affordance. */
  const TYPE_COLORS = {
    topic: "#5d35c7", person: "#59c2ff", fact: "#d5a20a",
    summary: "#2f86bd", other: "#8492a6",
  };
  ...
  function typeColor(type) {
    if (type === "summary") return themeColor("--graph-summary", TYPE_COLORS.summary);
      ...
      if (sourceCommunity !== targetCommunity) {
        ...
        if (!bundles[bundleKey]) {
          bundles[bundleKey] = {
            ...
            count: 0,
            weight: 0,
            color: edge.__color || typeColor("other"),
          };
        }
  ...
  Renderer.prototype.drawBackground = function(dark, animateDecorations) {
    ...
    ctx.save();
    ctx.fillStyle = themeColor("--graph-surface", dark ? "#211b26" : "#f8fbfe");
    ctx.fillRect(0, 0, this.width, this.height);
    ctx.fillStyle = hexToRgba(themeColor("--accent", dark ? "#ff5b96" : "#3c96ca"), dark ? 0.13 : 0.12);
    ...
    ctx.strokeStyle = hexToRgba(themeColor("--accent", dark ? "#ff5b96" : "#3c96ca"), dark ? 0.055 : 0.05);
    ctx.lineWidth = 1;
    ...
    if (animateDecorations) {
      ...
      ctx.fillStyle = hexToRgba(themeColor("--accent", dark ? "#ff5b96" : "#3c96ca"), dark ? 0.13 : 0.1);
  ...
  Renderer.prototype.render = function(nodes, edges, nodeMap, selection, hoverId, hoverType, layout, animProgress) {
    var ctx = this.ctx;
    ...
    var hoverNodeId = hoverType === "node" ? hoverId : null;
    var hoverEdgeId = hoverType === "edge" ? hoverId : null;
    ...
    /* Preserve the previous PersonalityRAG hover affordance on top of the
       LivingMemory 2.5.3 large-graph skeleton. Direct edges are appended so
       they remain visible even when the structural edge budget omitted them. */
    if (hoverNodeId !== null && !focusEdges) {
      var hoverEdges = this._nodeEdges[hoverNodeId] || [];
      if (hoverEdges.length) {
        var hoverEdgeIds = new Set();
        hoverEdges.forEach(function(edge) { hoverEdgeIds.add(edge.id); });
        visibleEdges = visibleEdges.filter(function(edge) {
          return !hoverEdgeIds.has(edge.id);
        }).concat(hoverEdges);
      }
    }
    ...
    for (var e = 0; e < visibleEdges.length; e++) {
      ...
      var isHoverConnected = hoverNodeId !== null &&
    ...
    for (var i = 0; i < nodes.length; i++) {
      ...
      var drawInfo = {
        ...
        isHovered: nd.id === hoverNodeId, isCenter: isCenter, hasFocus: hasNodeFocus,
        type: nd.type || "other", label: nd.label || "Unnamed",
        memoryCount: nd.memory_count || 0, degree: nd.degree || 0,
        labelScore: nd.labelScore || 0,
        color: typeColor(nd.type), hoverColor: hoverColor, fixed: nd.fixed,
      };
  ...
  Renderer.prototype._drawCommunities = function(nodes, layout, animProgress, dark) {
    ...
    var palette = ["#5d35c7", "#59c2ff", typeColor("summary"), "#d5a20a", "#8492a6", "#a594ff"];
    var ctx = this.ctx;
    ctx.save();
    ctx.setLineDash([5, 8]);
  ...
  Renderer.prototype._drawDenseNodeBuckets = function(ctx, buckets, dark) {
    Object.keys(buckets).forEach(function(key) {
      ...
      ctx.fillStyle = key === "muted"
        ...
        : hexToRgba(typeColor(key), dark ? 0.9 : 0.86);
      ctx.fill();
    });
  };
  ...
  Renderer.prototype._drawEdge = function(ctx, de, dark) {
    var opacity = de.isHoverConnected ? 0.9 : de.isHighlighted ? CFG.EDGE_OPACITY_HIGHLIGHT
      : de.hasFocus && de.isActive ? CFG.EDGE_OPACITY_ACTIVE : CFG.EDGE_OPACITY_DEFAULT;
    var width = de.isHoverConnected ? 1.8 : de.isHighlighted ? CFG.EDGE_WIDTH_HIGHLIGHT
    ...
    ctx.strokeStyle = de.isHoverConnected
      ? hexToRgba(de.hoverColor, opacity)
      : de.isHighlighted || (de.hasFocus && de.isActive)
  ...
  Renderer.prototype._drawParticles = function(ctx, de, now, dark) {
    var isEmphasized = de.isHighlighted || de.isHoverConnected;
    if (!de.isActive && !isEmphasized) return;
    var count = isEmphasized ? CFG.PARTICLE_COUNT_HIGHLIGHT
    ...
    for (var i = 0; i < count; i++) {
      ...
      ctx.beginPath();
      ctx.arc(px, py, CFG.PARTICLE_SIZE * (isEmphasized ? 1.35 : 1), 0, Math.PI * 2);
      ctx.fillStyle = hexToRgba(
        de.isHoverConnected ? de.hoverColor : de.color,
        isEmphasized ? 0.82 : 0.46
      );
      ctx.fill();
    }
  };
  ...
  Renderer.prototype._drawNode = function(ctx, dn, scale, dark) {
    ...
    if (halo > 0 && !dn.isMuted) {
      ...
      ctx.fillStyle = hexToRgba(
        dn.isHovered ? dn.hoverColor : dn.color,
        dn.isSelected ? 0.18 : dn.isHovered ? 0.14 : 0.055 + pulse * 0.035
      );
      ctx.fill();
    }
    ...
    ctx.strokeStyle = dn.isSelected || dn.isHovered || dn.isCenter
      ? (dn.isMuted ? (dark ? "#6f7683" : "#b9c0ca") : dn.isHovered ? dn.hoverColor : dn.color)
      : dark ? "#202126" : "#ffffff";
    ctx.stroke();
  ...
  Interaction.prototype._clearHover = function() {
    if (this._hoverId === null) return;
    this._hoverId = null;
    this._hoverType = null;
  ...
  Animator.prototype._tick = function() {
    ...
    if (dirty || this._needsRender) {
      ...
      var hoverType = this.interaction.getHoverType();
  ...
  Graph2D.prototype.getDiagnostics = function() {
    return {
      ...
      hoverType: this.interaction ? this.interaction.getHoverType() : null,
      hoverId: this.interaction ? this.interaction.getHoverId() : null,
      hoverConnectedEdges: this.renderer
        ? this.renderer._drawnEdges.filter(function(edge) { return edge.isHoverConnected; }).length
        : 0,
  ...
  function relationColor(type) {
    var palette = ["#59c2ff", "#d5a20a", "#2f86bd", "#5d35c7", "#8492a6"];
