...
def test_webui_entrypoint_is_only_shared_assembly() -> None:
    ...
    assert len(source.splitlines()) < 1_385
...
def test_docker_restart_waits_for_offline_transition_and_preserves_origin() -> None:
    ...
    assert "state.restart.sawOfflineTransition = false" in settings_source
    ...
    assert "requireOfflineTransition: false" in app_source
    assert "sawOfflineTransition: false" in app_source
